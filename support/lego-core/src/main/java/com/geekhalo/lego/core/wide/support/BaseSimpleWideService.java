...
public class BaseSimpleWideService<
    ...
    private void bindItemDatas(List<WIDE> wides) {
        ...
        for (WideItemDataProvider<TYPE, ? extends Object, ? extends WideItemData<TYPE, ?>> itemProvider : this.wideItemDataProviders) {
            ...
            for (WIDE wide : wides) {
                ...
                WideWrapper<WIDE> wideWrapper = wrapperCache.computeIfAbsent(wide, w -> this.wideWrapperFactory.createForWide(w));
    ...
    private Set<Object> getAllItemsKeys(List<WIDE> wides, TYPE supportType) {
        Set<Object> keys =  wides.stream()
                .flatMap(wide -> wide.getItemsKeyByType(supportType).stream())
                .filter(Objects::nonNull)
                .map(itemKey -> itemKey.getKey())
                .filter(Objects::nonNull)
    ...
    findWideItemDataProvider(TYPE itemType) {
        WideItemDataProvider<TYPE, Object, ? extends WideItemData<TYPE, ?>> wideItemProvider =
                (WideItemDataProvider<TYPE, Object, ? extends WideItemData<TYPE, ?>>)this.getWideItemDataProviders().stream()
