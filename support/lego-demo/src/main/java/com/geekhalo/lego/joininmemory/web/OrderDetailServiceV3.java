...
public class OrderDetailServiceV3 implements OrderDetailService{
    ...
    public List<? extends BaseOrderDetailVO> getByUserId(Long userId) {
        ...
        List<CommonAnnOrderDetailVO> orderDetailVOS = orders.stream()
                .map(order -> new CommonAnnOrderDetailVO(OrderVO.apply(order)))
                .collect(toList());

        this.joinService.joinInMemory(CommonAnnOrderDetailVO.class, orderDetailVOS);
